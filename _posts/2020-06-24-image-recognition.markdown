...
The final prediction layer exists as a dense layer: with the shape of the output being equal to the number of different image labels. In this layer the "softmax" function is used: which can be thought of as the probability that the network has determined the output neuron is the correct label based on the input data.
